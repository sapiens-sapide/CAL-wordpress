<?php
    ...
    function is_blog_installed ()
    {
        ...
        foreach ($wp_tables as $table) {
            ...
            // Die with a DB error.
            $wpdb->error = sprintf( /*WP_I18N_NO_TABLES*/
                'Une ou plusieurs tables de votre base de données sont indisponibles. La base de données a peut-être besoin d&rsquo;être <a href="%s">réparée</a>.' /*/WP_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed');
            dead_db();
        }
    ...
    function wp_upload_dir ($time = null)
    {
        ...
        $uploads = apply_filters('upload_dir', array('path'    => $dir,
                                                     'url'     => $url,
                                                     'subdir'  => $subdir,
                                                     'basedir' => $bdir,
                                                     'baseurl' => $burl,
    ...
    function wp_upload_bits ($name, $deprecated, $bits, $time = null)
    {
        ...
        $upload_bits_error = apply_filters('wp_upload_bits', array('name' => $name,
                                                                   'bits' => $bits,
                                                                   'time' => $time));
...
<?php
    ...
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
        <title><?php echo /*WP_I18N_DB_ERROR*/
        'Erreur de la base de données' /*/WP_I18N_DB_ERROR*/
        ; ?></title>

    </head>
    <body>
    <h1><?php echo /*WP_I18N_DB_CONNECTION_ERROR*/
    'Erreur lors de la connexion à la base de données' /*/WP_I18N_DB_CONNECTION_ERROR*/
    ; ?></h1>
    </body>
    </html>
    <?php
    ...
    function add_site_option ($option, $value)
    {
        ...
        if (!is_multisite()) {
            ...
            $_value = $value;
            $value = maybe_serialize($value);
            $result = $wpdb->insert($wpdb->sitemeta, array('site_id'    => $wpdb->siteid,
                                                           'meta_key'   => $option,
                                                           'meta_value' => $value));
            $value = $_value;
        }
    ...
    function update_site_option ($option, $value)
    {
        ...
        if (!is_multisite()) {
            ...
            $_value = $value;
            $value = maybe_serialize($value);
            $result = $wpdb->update($wpdb->sitemeta, array('meta_value' => $value), array('site_id'  => $wpdb->siteid,
                                                                                          'meta_key' => $option));
            $value = $_value;
        }
