<?php
    ...
    if (!$more_fields_settings->navigation || $more_fields_settings->navigation == 'boxes') {
        ...
        foreach ((array)$fields['_plugin'] as $key => $item) {
            ...
            $edit_link = array('navigation' => 'box',
                               'action'     => 'edit',
                               'keys'       => $keys);
            $delete_link = array('action'      => 'delete',
                                 'action_keys' => $keys,
                                 'class'       => 'more-common-delete');
            $export_link = array('navigation' => 'export',
                                 'keys'       => $keys);
            $warning = '';
            $data = array(
        ...
        if (!empty($fields['_plugin_saved'])) {
            ...
            foreach ($fields['_plugin_saved'] as $key => $item) {
                ...
                if (!$class) {
                    $class = (array_key_exists($key, $fields['_plugin'])) ? 'disabled' : false;
                }

                $edit_link = array('navigation' => 'box',
                                   'action'     => 'edit',
                                   'keys'       => '_plugin_saved,' . $key);
                ...
                if ($class) {
                    $data = array($label, count($item['fields']), __('Overridden above', 'more-plugins'));
                }
        ...
        if (!empty($fields['_other'])) {
            ...
            foreach ($fields['_other'] as $key => $item) {
                ...
                if ($class) {
                    $data = array($label, __('Overridden above', 'more-plugins'));
                }
                $more_fields_settings->table_row($data, $nbr++, $class);
            }
            $more_fields_settings->table_footer($titles);
        ...
        if ($more_fields_settings->navigation == 'box') {
            ...
            $more_fields_settings->settings_form_header(array('navigation' => 'boxes',
                                                              'action'     => 'save'));
	...
	<?php
            ...
            foreach ($remove as $r) if ($types[$r]) {
                unset($types[$r]);
            }
            ...
            if (is_object($more_types) && !empty($more_types)) {
                ...
                foreach ($mftss as $key => $mft) {
                    ...
                    if (array_key_exists($key, (array)$mfts)) {
                        $options[$key] = array_merge($options[$key], array('disabled' => true,
                                                                           'text'     => __("If you want to use your box for this post type you need to import it using <a href='$link'>More Types</a>.", 'more-types')));
                    }
                }
            }
            ...
            if ($more_fields_settings->action != 'add') {
                ...
                foreach ((array)$fields as $key => $item) {
                    ...
                    $edit_link = array('navigation' => 'field',
                                       'action'     => 'edit',
                                       'keys'       => $akeys);
                    $delete_link = array('navigation'  => 'box',
                                         'action'      => 'delete',
                                         'keys'        => $keys,
                                         'class'       => 'more-common-delete',
                                         'action_keys' => $akeys);
                    ...
                    $row = array(
                        ...
                        $more_fields_settings->settings_link('Edit', $edit_link) . ' | ' .
                            ...
                            $more_fields_settings->updown_link($nbr, $total, array('keys'        => $box,
                                                                                   'action_keys' => array($box, 'fields')))
                    );
        ...
        <div class="more-plugins-advanced-settings">
            ...
            <div class="more-advanced-settings">
                <table class="form-table">
                    ...
                    $comment = '<em>' . $comment . '</em>';
                    $select = array('right' => __('Right', 'more-plugins'),
                                    'left'  => __('Left', 'more-plugins'));
        ...
        <?php
            ...
            if ($more_fields_settings->navigation == 'field') {
                //	$data = $more_fields_settings->read_data();
                //	$data = $more_fields_settings->get_data();
                $more_fields_settings->default = array('position'   => 'right',
                                                       'field_type' => 'text');
                ...
                if (!$box) {
                    $box = $more_fields_settings->get_val('label', array($keys[1]));
                }
                if (!$navtext) {
                    $navtext = __('Add new', 'more-plugins');
                }

                $navurl = $more_fields_settings->settings_link($box, array('keys'       => $keys[0] . ',' . $keys[1],
                                                                           'navigation' => 'box'));
                $more_fields_settings->navigation_bar(array($navurl, $navtext));

                $form_link = array('navigation'  => 'box',
                                   'keys'        => $keys[0] . ',' . $keys[1],
                                   'action_keys' => $keys,
                                   'action'      => 'save');
