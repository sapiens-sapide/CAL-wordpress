<?php
    ...
    */
    function get_box ($name, $args = array())
    {
        ...
        if (!is_array($more_fields)) {
            return '<!-- No More Fields are defined -->';
        }
        if (!array_key_exists($name, $more_fields)) {
            return "<!-- The box name '$name' does not exist! -->";
        }

        // Make sure we've got someting to display
        $something = false;
        foreach ((array)$more_fields[$name]['field'] as $field)
            if (get_post_meta($post->ID, $field['key'], true)) {
                $something = true;
            }
        if (!$something) {
            return "<!-- Nothing to display for '$name' -->";
        }
        ...
        for ($i = 0; $i < count($more_fields[$name]['field']); $i++) {
            ...
            if ($show && $value) {
                ...
                if ($args['format'] == 'ul') {
                    $html .= "<li ${style_li}>" . $value . '</li>' . $nl;
                }
                else {
                    if ($args['format'] == 'dl') {
                        $html .= "<dt ${style_dt}>" . $title . "</dt><dd ${style_dd}>" . $value . '</dd>' . $nl;
                    }
                    else {
                        if ($args['format'] == 'p') {
                            $html .= $value . $nl;
    ...
    */
    function more_fields_img ($meta, $before = '', $after = '', $options = array())
    {
        ...
        // Churn out some HTML
        $attr = array('class' => 'mf_image attachment-' . $id,
                      'id'    => 'attachment-' . $id);
        $b = wp_get_attachment_image($id, $size, false, $attr);
        if ($new_file) {
            $b = str_replace($original_file, $new_file, $b);
        }
        echo $before . $b . $after;
