    ...
    class more_types_admin extends more_plugins_admin_object_sputnik_8
    {
        ...
        function add_actions ()
        {
            ...
            foreach ($taxonomies as $tax) {
                if (!$tax->hierarchical) {
                    array_push($this->fields['var'], 'default_taxonomy_' . $tax->name);
                }
                else {
                    array_push($this->fields['array'], 'default_taxonomy_' . $tax->name);
                }
            }
            ...
            if ($this->action == 'save') {
                ...
                if (array_key_exists('taxonomies', $_POST)) {
                    foreach ((array)$_POST['taxonomies'] as $t) {
                        $a = get_taxonomy($t);
                        if ($a->hierarchical) {
                            $this->fields['array'][] = 'default_taxonomy_' . $t;
                        }
                        else {
                            $this->fields['var'][] = 'default_taxonomy_' . $t;
                        }
                    }
                }
                //__d($a);
                ...
                if (array_key_exists($key, $pt)) {
                    if (is_array($pt[$key])) {
                        if (in_array($value, $pt[$key])) {
                            ...
                            $ret = array_merge($ret, array('value' => ''));
                        }
