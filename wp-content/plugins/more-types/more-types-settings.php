<?php
    ...
    if (!$this->navigation || $this->navigation == 'post_types') {
        ...
        foreach ($types['_plugin'] as $name => $tax) {
            ...
            $edit_link = array('navigation' => 'post_type',
                               'keys'       => $keys);
        ...
        if (!empty($types['_plugin_saved'])) {
            ...
            foreach ($types['_plugin_saved'] as $name => $tax) {
                ...
                if (!$class) {
                    $class = (array_key_exists($name, $types['_plugin'])) ? 'disabled' : false;
                }
        ...
        if (!empty($types['_default'])) {
            ...
            foreach ($types['_default'] as $name => $tax) {
                ...
                $edit_link = array('navigation' => 'post_type',
                                   'keys'       => $keys);
                $hide_link = array('action'      => 'disable',
                                   'action_keys' => $keys,
                                   'class'       => 'more-common-delete');
                $export_link = array('navigation' => 'export',
                                     'keys'       => $keys);
        ...
        if ($this->navigation == 'post_type') {
            ...
            $more_types_settings->settings_form_header(array('navigation' => 'post_types',
                                                             'action'     => 'save'));
        ...
        <table class="form-table">
            ...
            */
            ...
            if (!empty($boxes)) {
                $row = array(__('Additional boxes', 'more-plugins'), $more_types_settings->checkbox_list('boxes', $boxes));
            }
            else {
            ...
            foreach ($wp_taxonomies as $taxonomy) {
                if (!trim($taxonomy->label)) {
                    continue;
                }
                $tax[$taxonomy->name] = $taxonomy->label;
                if (in_array($taxonomy->name, (array)$mytax)) {
                    $stax[] = $taxonomy;
                }
            }
